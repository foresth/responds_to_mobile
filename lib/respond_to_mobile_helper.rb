module RespondToMobileHelper
  ACCEPTABLE_TYPES = [:mobile, :basic]
  
  def mobile_xhtml_doctype(type = :mobile, version = '1.0')
    raise Exception.new("RespondToMobile: XHTML DOCTYPE type must either be ':mobile' or ':basic'") unless ACCEPTABLE_TYPES.include?(type)
    raise Exception.new("RespondToMobile: XHTML DOCTYPE version must be in the format of '1.0' or '1.1', etc.") unless version.include?('.')
